...
class MLCP_Admin {
    ...
    public static function save_settings() {
        if (!current_user_can('manage_options')) {
            wp_die(__('Acesso negado.', MLCP_TEXT_DOMAIN));
        }
        ...
        $settings = array(
            ...
            'default_overlay_opacity' => sanitize_text_field(wp_unslash($_POST['default_overlay_opacity'] ?? '0.42')),
    ...
    public static function render_item_columns($column, $post_id) {
        ...
        if ($column === 'mlcp_image') {
            ...
            echo $img ? '<img src="' . esc_url($img) . '" style="width:72px;height:48px;object-fit:cover;border-radius:8px;" alt="" />' : '—';
        } elseif ($column === 'mlcp_subtitle') {
            echo $meta['subtitle_display'] ? esc_html($meta['subtitle_display']) : '—';
    ...
    public static function handle_duplicate_item() {
        ...
        if (!$post_id) {
            wp_die(__('Item invÃ¡lido.', MLCP_TEXT_DOMAIN));
        }
        ...
        $new_post['post_title'] = $post['post_title'] ? $post['post_title'] . ' - CÃ³pia' : __('CÃ³pia do item', MLCP_TEXT_DOMAIN);
        $new_post['post_name'] = '';
